...
class TreasureAngleToWorldmapPositionConverter:
    ...
    def __convert_to_map_position__(self):
        ...
        self.camera_position = self.__compute_camera_position__()
        ...
        detected_treasures = []
        for angle in self.treasure_angles:
            treasure_slope = -math.tan(math.radians(self.robot_angle-(63.53/2)+angle))
            treasure_offset = self.camera_position[1] - (treasure_slope * self.camera_position[0])
            ...
            distance_bottom = self.__find_distance_between__(self.camera_position, (treasure_bottom_x, treasure_bottom_y))
            distance_top = self.__find_distance_between__(self.camera_position, (treasure_top_x, treasure_top_y))
            distance_rear = self.__find_distance_between__(self.camera_position, (treasure_rear_x, treasure_rear_y))

            min_list = []
            if treasure_bottom_x < self.camera_position[0]:
                min_list.append(distance_bottom)
            if treasure_top_x < self.camera_position[0]:
    ...
    def __find_distance_between__(self, first_point, second_point):
