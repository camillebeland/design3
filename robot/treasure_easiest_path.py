...
class TreasureEasiestPath:
    ...
    def find_easiest_treasure_from(self, position, target_island_position):
        ...
        paths = list(map(lambda pos: {'length': len(self.__pathfinder.find_path(position, pos)) +
                                                 len(self.__pathfinder.find_path(pos, target_island_position)),
                                       'treasure': pos}, treasures_positions))
